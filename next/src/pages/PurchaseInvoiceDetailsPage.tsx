...
export default function PurchaseInvoiceDetailsPage() {
  ...
  const { data: invoice, isLoading, error, mutate } = useFrappeGetDoc<PurchaseInvoice>("Purchase Invoice", name);
  ...
  if (!invoice) {
    return <div>Purchase Invoice not found.</div>;
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex space-x-2">
          {invoice.docstatus !== 2 && (
            <>
              ...
              {!invoice.is_paid && <Button variant="outline"><CreditCard className="mr-2 h-4 w-4" />Create Payment</Button>}
              <AlertDialog>
      ...
      <Card className="relative">
        ...
        <CardHeader>
          <CardTitle>Purchase Invoice : {invoice.name}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div className="flex justify-end">
              <div className="space-y-2">
                <p className="flex items-center justify-between"><strong>Is Paid:</strong> <Badge variant="outline" className={`w-24 text-center justify-center ${invoice.is_paid ? "border-green-500 text-green-500" : "border-red-500 text-red-500"}`}>{invoice.is_paid ? "Yes" : "No"}</Badge></p>
                <p className="flex items-center justify-between"><strong>Update Stock:</strong> <Badge variant="outline" className={`w-24 text-center justify-center ${invoice.update_stock ? "border-green-500 text-green-500" : "border-red-500 text-red-500"}`}>{invoice.update_stock ? "Yes" : "No"}</Badge></p>
                {invoice.update_stock === 1 && <p><strong>Warehouse:</strong> {invoice.set_warehouse}</p>}
              </div>
            </div>
          </div>
